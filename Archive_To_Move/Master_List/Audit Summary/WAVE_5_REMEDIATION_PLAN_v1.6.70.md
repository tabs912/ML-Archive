...
## Hybrid Tracking Update — Function Governance Registry

The hybrid method is now represented by `Master_List/Audit_Summary/FUNCTION_GOVERNANCE_REGISTRY_v1.6.70.csv`.

This registry combines generated inventory evidence with human-governance fields so the project can track:

- no-static-path status;
- undefined/dead-path review status;
- compatibility-wrapper status;
- approved action;
- proposed Phase 2 batch;
- required consumer checks;
- required testing.

## Aggressive Cleanup / Single Full Test-Pass Strategy

The preferred strategy is to avoid many repeated full workflow test passes. To support that, Phase 2 should group as many **approved low-risk internal cleanup candidates** as possible into one controlled cleanup batch, then run the full staging test suite once for that batch.

### Important Safety Rule

Do **not** remove every no-static-path function automatically. No-static-path means only that the static scan did not find a direct path. It does not rule out manual execution, dynamic string invocation, menu/trigger use, `google.script.run`, external spreadsheet consumers, library consumers, or diagnostic use.

### Recommended Aggressive Batch Gate

A function may enter the aggressive cleanup batch only when all are true:

1. It is not listed as a supported public API or trigger entry point in `PUBLIC_API_BOUNDARY_v1.6.68.md`.
2. It is not a menu callback.
3. It has no approved compatibility-wrapper requirement.
4. It has no direct static callers or string references.
5. It is not required by Dashboard Quality, Framework Health, or staging diagnostics.
6. External/manual/library consumer risk has been reviewed or accepted by the user.
7. The user approves it for the aggressive cleanup batch.

### One-Pass Cleanup Verification

After the approved cleanup batch is applied, run the full v1.6.70 staging checklist once:

1. Static syntax and callback scans.
2. Startup/menu tests.
3. Template and Dashboard Quality tests.
4. Individual workflow tests.
5. End-to-end `runMonthlyUpdate`.
6. Framework Timing verification.
7. Public API boundary verification.
8. Archive/data-safety tests.
9. Rollback readiness check.

If failures appear, correct them as one follow-up stabilization batch rather than running the full test suite after each individual removal.

## Recommended Next Implementation Step

Before production code removal, review `FUNCTION_GOVERNANCE_REGISTRY_v1.6.70.csv` and select the first aggressive cleanup batch from rows with:

- `approved_action = CANDIDATE_FOR_AGGRESSIVE_REMOVAL_REVIEW`
- `phase2_batch = AGGRESSIVE_INTERNAL_CANDIDATE`
- no user objection after review.

Only after that approval should a new script version remove or wrap functions.
