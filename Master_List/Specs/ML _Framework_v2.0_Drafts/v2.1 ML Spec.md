...
**Saved artifact:** **Master\_List/Specs/Master\_List\_Framework\_Specification\_v2.0\_UPDATEDv2.md**.

**Source update file:** **Master\_List/Specs/Master\_List\_Framework\_Specification\_v2.0.md**.

**Authoritative implementation:** **Master\_List/Current Production Script/v.1.7.6\_Current\_Production\_Script**.
...
| :---- | :---- |
| Current production script | **v.1.7.6\_Current\_Production\_Script** is the implementation authority. |
...
| Document properties | RFF\_ARCHIVE\_SPREADSHEET\_ID, ML\_INDEX\_RESTORE\_WEB\_APP\_URL, template signatures, Index signatures, Dashboard Quality section state, workflow-busy state, and deferred-index state. | Deployment/runtime configuration and persistent operational state. |
...
| Header row | HEADER\_ROW is row 4\. |
| Data start row | DATA\_START\_ROW is row 5\. |
...
| Templates | **Template \- Banner Report**, **Template \- Care Plan Due**, **Template \- Unlocked Care Plan**, **Template \- Raw Data**, **Template \- Demo P**, **Template \- Disenrolled Exclusion**, **Template \- Master List**, **Template \- Monthly Change**, RFF\_BASE\_TEMPLATE |
...
10\. System/template tail block: Framework Timing Report, Dashboard Quality Report, Format Dashboard, governed templates, and RFF\_BASE\_TEMPLATE.
...
Timing wrappers preserve performance evidence through runFrameworkTimed\_, runtime timing helpers, framework timing report sections, thresholds, recommendations, compact section updates, and retention limits. Timing evidence feeds Dashboard Quality Performance Summary and Framework Health reporting.
...
* **ARCHITECTURE\_INVENTORY\_v1.6.29.md**  
* **CACHE\_INVENTORY\_v1.6.29.md**  
* **DASHBOARD\_INVENTORY\_v1.6.29.md**  
* **ENGINEERING\_GOVERNANCE\_REFERENCE\_v1.6.29.md**  
* **ENGINEERING\_GOVERNANCE\_REFERENCE\_PART\_VI\_v1.6.29.md**  
* **FRAMEWORK\_RULES\_CATALOG\_v1.6.29.md**  
* **WRAPPER\_INVENTORY\_v1.6.29.md**  
* **CONFIGURATION\_INVENTORY\_v1.6.29.md**  
* **FUNCTION\_CALL\_GRAPH\_v1.6.29.md**  
* **HELPER\_CATALOG\_v1.6.29.md**  
* **PRODUCTION\_DATA\_FLOW\_v1.6.29.md**  
* **TEMPLATE\_INVENTORY\_v1.6.29.md**  
* **VALIDATION\_CATALOG\_v1.6.29.md**  
* **WORKSHEET\_INVENTORY\_v1.6.29.md**

\---

# **Appendix H — Document Formatting Specification**

This appendix defines the required formatting for generated Google Docs, DOCX files, and PDF exports of the Master List Framework Specification. The Markdown file remains the editable master source. Generated documents must follow these standards and must not be independently edited in a way that changes governing content.

## **H.1 Source and Build Rules**

| Item | Standard |
| :---- | :---- |
| Master source | The Markdown specification is the authoritative editable document source. |
| Generated documents | Google Docs, DOCX files, and PDFs are generated presentation artifacts. |
| Editing rule | Content changes are made in the Markdown source first. |
| Build consistency | Every generated document must apply this appendix consistently. |
| Independent edits | Generated documents must not contain content changes that are absent from the Markdown master. |
| Quality review | Generated DOCX files must be rendered and visually reviewed before release. |
...
| Orientation | Portrait unless a wide reference table requires landscape orientation. |
| Top margin | 0.70 inch. |
| Bottom margin | 0.70 inch. |
| Left margin | 0.75 inch. |
| Right margin | 0.75 inch. |
| Page numbers | Centered in the footer. |
| Major section breaks | Use page breaks only where they improve navigation or prevent awkward page starts. |

## **H.3 Title and Heading Styles**

| Style | Font | Size | Color | Weight / Style | Space Before | Space After |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| Title | Calibri | 26 pt | **\#366091** | Bold | 26 pt | 26 pt |
| Subtitle | Calibri | 12 pt | **\#4F81BD** | Italic | 12 pt | 12 pt |
| Heading 1 | Calibri | 20 pt | **\#4F81BD** | Bold | 20 pt | 20 pt |
| Heading 2 | Calibri | 16 pt | **\#4F81BD** | Bold | 16 pt | 16 pt |
| Heading 3 | Calibri | 14 pt | **\#4F81BD** | Bold | 14 pt | 14 pt |
| Heading 4 | Calibri | 12 pt | **\#4F81BD** | Bold | 12 pt | 12 pt |

Heading rules:

* The document title uses the Title style.  
* Major numbered sections use Heading 1\.  
* Subsections use Heading 2 through Heading 4 according to hierarchy.  
* Headings must remain with the paragraph that follows them.  
* Heading levels must not be skipped.  
* Headings must not be manually formatted with Normal style.
...
| Space after | 5 pt. |
| Alignment | Left. |
| Manual line breaks | Avoid unless required for meaning or layout. |
...
| Bullet lists | Use the standard bullet-list style. |
| Numbered lists | Preserve the numbering shown in the Markdown source. |
| Nested lists | Increase indentation consistently in 0.25-inch increments. |
...
| Border width | 0.5 pt. |
| Header fill | **\#C7D8EA**. |
| Header font | Calibri Bold, 12 pt. |
| Header font color | **\#366091**. |
| Body font | Cambria, 11 pt. |
| Vertical alignment | Top. |
| Header rows | Repeat at the top of each page when a table spans pages. |
| Cell padding | Use consistent compact padding that preserves readability. |
| Width | Fit within page margins. |
| Markdown conversion | Markdown tables must be converted into true document tables. |
| Blank rows | Do not insert blank rows inside a table unless they carry meaning. |

Table rules:

* The first row is the table header.  
* Header styling must not be applied to body rows.  
* Wide tables may use landscape orientation when necessary.  
* Tables must not extend beyond page margins.  
* Tables should not be split in a way that leaves a header row alone at the bottom of a page.  
* Repeating table headers must be structurally enabled, not simulated by copied rows.

## **H.7 Framework Name and Identifier Formatting**

| Content Type | Required Formatting |
| :---- | :---- |
| File names | Cambria 11 pt, bold. |
| File paths | Cambria 11 pt, bold. |
| Sheet names | Cambria 11 pt, bold. |
| Template sheet names | Cambria 11 pt, bold. |
| Dashboard section names | Cambria 11 pt, bold. |
| Menu commands | Cambria 11 pt, bold. |
| Apps Script functions | Consolas, 10.5 pt. |
| Apps Script constants | Consolas, 10.5 pt. |
| Document-property keys | Consolas, 10.5 pt. |
| Code blocks | Consolas, 10.5 pt. |

Examples:

| Example | Classification | Formatting |
| :---- | :---- | :---- |
| **v.1.7.6\_Current\_Production\_Script** | File name | Bold Cambria. |
| **Master\_List\_Framework\_Specification\_v2.0\_UPDATEDv3.md** | File name | Bold Cambria. |
| **Master List** | Sheet name | Bold Cambria. |
| **Template \- Master List** | Sheet name | Bold Cambria. |
| **SECTION A \- GLOBAL SETTINGS** | Dashboard section name | Bold Cambria. |
| **Create Monthly Update** | Menu command | Bold Cambria. |
| runMonthlyUpdate() | Apps Script function | Consolas. |
| createMasterList() | Apps Script function | Consolas. |
| HEADER\_ROW | Apps Script constant | Consolas. |
| RFF\_ARCHIVE\_SPREADSHEET\_ID | Document-property key | Consolas. |

Classification rules:

* A name beginning with **Template \-** is a sheet name unless the context explicitly refers to a code constant or object.  
* Backticks in Markdown do not automatically mean monospace in the generated document.  
* The builder must classify the enclosed text before assigning typography.  
* File names, paths, sheet names, template sheet names, dashboard section names, and menu commands use bold Cambria.  
* Functions, constants, document-property keys, and actual code use Consolas.

## **H.8 Color Standards**

| Use | Color |
| :---- | :---- |
| Document title | **\#366091**. |
| Headings and subtitle | **\#4F81BD**. |
| Table header fill | **\#C7D8EA**. |
| Table header text | **\#366091**. |
| Normal text | Automatic black. |

No additional decorative colors should be introduced without updating this appendix.

## **H.9 Google Docs Compatibility**

Generated documents must remain compatible with Google Docs.

* Do not depend on Word-only layout features for essential content.  
* Use standard fonts available in Google Docs.  
* Use true heading styles.  
* Use true tables.  
* Use standard bullets and numbering.  
* Avoid floating text boxes, unsupported fields, or complex anchored objects.  
* Verify that table headers, spacing, and hierarchy remain understandable after import to Google Docs.

## **H.10 Build Verification Checklist**

Before publishing a generated document:

1\. Confirm the Markdown source is the current approved source.

2\. Confirm the title and all heading levels match Section H.3.

3\. Confirm Normal text uses Cambria 11 pt, 1.15 line spacing, and 5 pt before and after.

4\. Confirm lists use a 0.25-inch indent.

5\. Confirm Markdown tables were converted to true tables.

6\. Confirm table header rows repeat across pages.

7\. Confirm file names and sheet names use bold Cambria.

8\. Confirm Apps Script functions, constants, property keys, and code use Consolas.

9\. Confirm page numbers and margins are correct.

10\. Render the DOCX and inspect every page for clipping, overlap, broken tables, orphaned headings, and inconsistent spacing.

11\. Correct defects and repeat the render review before release.

## **H.11 Governance**

This appendix is the authoritative formatting specification for generated Master List Framework documentation. Any change to document typography, spacing, table design, identifier formatting, page layout, or build behavior must be recorded here before the revised formatting is treated as approved.
