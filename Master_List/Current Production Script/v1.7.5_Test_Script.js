...
const MASTER_LIST_MERGE_ML_VERSION = "1.7.5";
...
function writeDashboardDefaultsFast_(sheet) {
  ...
  appendDashboardSectionRows_(rows, RFF_SECTION_BEHAVIORS, ["Sheet Type", "Uses Title Rows", "Uses Filter", "Uses Alternating Colors", "Uses Subheaders", "Hidden Template", "Output Visibility"], getDefaultBehaviorRows_(), width);
  appendDashboardSectionRows_(rows, RFF_SECTION_SYSTEM_SURFACES, ["System Sheet Name", "Display Name", "Sort Order", "Output Visibility", "Title Fill Color", "Title Font Color", "Use Global Defaults", "Notes"], getDefaultSystemSurfaceRows_(), width);
...
function getGovernedNewSheetInsertIndex_(ss) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  const frameworkTiming = ss.getSheetByName(RFF_TIMING_SHEET) || ss.getSheetByName("Framework Timing Report");
  if (frameworkTiming) return Math.max(frameworkTiming.getIndex() - 1, 1);
  ...
  let firstProtectedIndex = null;
  protectedNames.forEach(function(sheetName) {
    const sheet = ss.getSheetByName(sheetName);
    if (!sheet) return;
    const idx = sheet.getIndex();
    if (firstProtectedIndex === null || idx < firstProtectedIndex) firstProtectedIndex = idx;
  });
  if (firstProtectedIndex === null) return ss.getNumSheets();
  return Math.max(firstProtectedIndex - 1, 1);
}

function insertGovernedOutputSheet_(ss, sheetName) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  return ss.insertSheet(sheetName, getGovernedSheetInsertIndex_(ss, sheetName));
}

function getGovernedSheetInsertIndex_(ss, sheetName) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  const targetName = String(sheetName || "").trim();
  const targetRank = getGlobalSheetSortRankByName_(targetName);
  let maxAllowedIndex = ss.getNumSheets() + 1;
  if (targetRank < 150) {
    const timingSheet = ss.getSheetByName("Framework Timing Report") || ss.getSheetByName(RFF_TIMING_SHEET);
    if (timingSheet) maxAllowedIndex = timingSheet.getIndex();
  }
  let targetIndex = targetRank > 20 ? 3 : 1;
  ss.getSheets().forEach(function(sheet) {
    const name = String(sheet.getName() || "").trim();
    if (!name || name === targetName) return;
    const rank = getGlobalSheetSortRankByName_(name);
    if (rank < targetRank) targetIndex = Math.max(targetIndex, sheet.getIndex() + 1);
    if (rank === targetRank && compareSheetNamesByGlobalOrder_(name, targetName) < 0) {
      targetIndex = Math.max(targetIndex, sheet.getIndex() + 1);
    }
  });
  return Math.max(1, Math.min(targetIndex, maxAllowedIndex));
}

function getOperationalNewSheetInsertIndex_(ss, sheetName) {
  return getGovernedSheetInsertIndex_(ss, sheetName);
}
...
function handleFormatDashboardValueHighlighting_(e) {
  ...
  const endCol = Math.min(editedRange.getLastColumn(), 14);
...
function upsertDashboardColumnDefinitionRows_(dashboardSheet, columnRows) {
  ...
  if (!bounds) throw new Error("Format Dashboard Section F Column Definitions was not found.");
...
const RFF_SECTION_COLUMNS = "SECTION F - COLUMN DEFINITIONS";

const RFF_SECTION_HEADERS = "SECTION G - SHEET HEADERS";
...
function getDefaultSheetDefinitionRows_() {
  return [
  ["Banners", "Banner Report", "Template - Banner Report", "Banners mm.yy", "#65A9CC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25, 0, false],
  ["CP Due Date", "Care Plan Due Date Report", "Template - Care Plan Due", "CP Due mm.yy", "#65CC99", true, "Pulled From Spreadsheet", 100, "FIXED", 100, 25, 0, false],
  ["Unlock CP", "Unlocked Care Plan Report", "Template - Unlocked Care Plan", "Unlock CP mm.yy", "#65CCC3", true, "Pulled From Spreadsheet", 100, "FIXED", 100, 25, 0, false],
  ["Raw Data", "Raw Data", "Template - Raw Data", "Raw Data mm.yy", "#657FCC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25, 0, false],
  ["Demo P", "Demo P", "Template - Demo P", "Demo P", "#657FCC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25, 0, false],
  ["Disenrolled Exclusion", "Disenrolled Exclusion", "Template - Disenrolled Exclusion", "Disenrolled", "#CC65A1", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25, 0, false],
  ["Master List", "Master List", "Template - Master List", "Master List mm.yy", "#7665CC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25, 0, false],
  ["Monthly Change", "Monthly Change Report", "Template - Monthly Change", "Monthly Change mm.yy", "#A165CC", true, "Last Day of Prompt Month", 100, "FIXED", 100, 25, 0, false]
];
}
...
function getDefaultSystemSurfaceRows_() {
  return [
    ...
    [DEMO_P_ARCHIVE_SHEET, DEMO_P_ARCHIVE_SHEET, 75, "HIDDEN", "#657FCC", "#000000", true, "Hidden Demo P row archive"],
...
function createOrRefreshAllReportTemplates() {
  return runFrameworkTimed_("Create / Refresh All Templates", function(timing) {
    ...
    organizeTemplateTabsBeforeBaseTemplate_(dashboard, timing);
    ...
    markFrameworkStep_(timing, "Template tabs organized between Format Dashboard and RFF_BASE_TEMPLATE without global sort");
...
function moveTemplateBeforeBaseTemplate_(sheet, timing) {
  if (!sheet || !String(sheet.getName() || "").startsWith("Template - ")) return false;
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const baseSheet = ss.getSheetByName(RFF_BASE_TEMPLATE_NAME);
  if (!baseSheet || baseSheet.getSheetId() === sheet.getSheetId()) return false;
  try {
    const targetIndex = getGovernedSheetInsertIndex_(ss, sheet.getName());
    if (sheet.getIndex() === targetIndex || sheet.getIndex() === targetIndex - 1) return false;
    if (typeof sheet.isSheetHidden === "function" && sheet.isSheetHidden()) return false;
    ss.setActiveSheet(sheet);
    ss.moveActiveSheet(Math.min(targetIndex, ss.getNumSheets()));
    if (timing) markFrameworkStep_(timing, "Template positioned in governed order: " + sheet.getName());
    return true;
  } catch (err) {
    logBestEffortWarning_("Template tab positioning skipped for " + sheet.getName() + ": " + err.message);
    return false;
  }
}

function organizeTemplateTabsBeforeBaseTemplate_(dashboard, timing) {
  let moved = 0;
  sortSheetDefinitionsByProductionOrder_((dashboard && dashboard.sheetDefinitions) || []).forEach(function(sheetDef) {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetDef.templateName);
    if (moveTemplateBeforeBaseTemplate_(sheet, timing)) moved++;
  });
  forceBaseTemplateHidden_();
  if (timing) markFrameworkStep_(timing, "Template block positioned before RFF_BASE_TEMPLATE | Moves: " + moved);
  return moved;
}
...
function createOrRefreshTemplateFromDashboard_(dashboard, sheetDef, timing) {
  ...
  if (!sheet) {
    ...
    moveTemplateBeforeBaseTemplate_(sheet, timing);
  }
...
function promoteStagedTemplateBuild_(ss, buildSheet, existingSheet, sheetDef, timing) {
  ...
  moveTemplateBeforeBaseTemplate_(buildSheet, timing);
...
function formatMonthlyBannerSheet_(rawSheet, parsedMonth, timing) {
  ...
  positionSheetInGlobalSortOrder_(outputSheet);
...
function formatBannerReport() {
  return runFrameworkTimed_("Format Banner Report", function(timing) {
    ...
    positionSheetInGlobalSortOrder_(outputSheet);
    showSheetIfNeeded_(outputSheet, timing, "Banner output sheet shown: " + outputName);
    markFrameworkStep_(timing, "Template duplicated as output sheet in governed order: " + outputName);
...
function archiveActiveRawDataSheet() {
  return runFrameworkTimed_("Archive Active Raw Data Sheet", function(timing) {
    ...
    archiveRawDataSheet_(sheet, sheet.getName(), timing, "Active archive detail");
...
function refreshIndexAfterSheetWorkflow_(workflowName) {
  try {
    createIndexSheet();
  } catch (err) {
...
function archiveRawSourceAndDeleteLocal_(ss, rawSheet, archiveName, outputSheet, timing) {
  if (RFF_ENABLE_AUTO_ARCHIVE_RAW_DATA === true) {
    try {
      archiveRawDataSheet_(rawSheet, archiveName, timing, "Raw archive detail");
...
function archiveRawDataSheet_(sourceSheet, archiveName, timing, timingLabel) {
  ...
  markArchiveStep("archive workbook opened", "Archive Spreadsheet ID: " + archiveId);
...
function hideMonthlyImportSheets() {
  const monthParts = promptForLockedYearReportMonth_("Hide Monthly Import Sheets");
  if (!monthParts) return null;
  const parsed = buildPromptedMonthContext_(monthParts);
  const specs = [
  ...
  return hideMonthlySheetsBySpecs_("Hide Monthly Import Sheets", parsed, specs);
}

function hideMonthlyActiveSheets() {
  const monthParts = promptForLockedYearReportMonth_("Hide Monthly Active Sheets");
  if (!monthParts) return null;
  const parsed = buildPromptedMonthContext_(monthParts);
  const specs = [
  ...
  return hideMonthlySheetsBySpecs_("Hide Monthly Active Sheets", parsed, specs);
}

function hideMonthlySheetsBySpecs_(processName, parsedMonth, specs) {
  return runFrameworkTimed_(processName + " " + parsedMonth.monthLabel, function(timing) {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const result = { hidden: [], skipped: [] };
    specs.forEach(function(spec) {
      const sheets = findArchiveMonthlyCandidateSheets_(ss, spec.keywords, parsedMonth.monthLabel);
      if (!sheets.length) {
    ...
    enforceGlobalSheetSortOrder_(parsedMonth.monthParts);
    notify_(processName + " complete for " + parsedMonth.monthLabel + ". Hidden: " + result.hidden.length + "; Skipped: " + result.skipped.length + ".");
    return result;
  });
}

function archiveMonthlyImportSheets() {
  const monthParts = promptForLockedYearReportMonth_("Archive Monthly Import Sheets");
  if (!monthParts) return null;
  const parsed = buildPromptedMonthContext_(monthParts);

  return runFrameworkTimed_("Archive Monthly Import Sheets " + parsed.monthLabel, function(timing) {
    const result = archiveMonthlySheetsBySpecs_(parsed, [
      { label: "Banners", keywords: ["Banners", "Banner Report"] },
      { label: "CP Due Date", keywords: ["CP Due Date", "CP Due", "Care Plan Due Date Report", "Care Plan Due"] },
      { label: "Unlocked CP", keywords: ["Unlocked CP", "Unlock CP", "Unlocked Care Plan"] }
    ], timing);
    refreshIndexAfterSheetWorkflow_("Archive Monthly Import Sheets");
    notifyArchiveMonthlySheetsResult_("Archive Monthly Import Sheets", parsed.monthLabel, result);
    return result;
  });
}

function archiveMonthlyActiveSheets() {
  const monthParts = promptForLockedYearReportMonth_("Archive Monthly Active Sheets");
  if (!monthParts) return null;
  const parsed = buildPromptedMonthContext_(monthParts);

  return runFrameworkTimed_("Archive Monthly Active Sheets " + parsed.monthLabel, function(timing) {
    const result = archiveMonthlySheetsBySpecs_(parsed, [
      { label: "Raw Data", keywords: ["Raw Data"] },
      { label: "Master List", keywords: ["Master List"] },
      { label: "Monthly Change", keywords: ["Monthly Change"] }
    ...
    notifyArchiveMonthlySheetsResult_("Archive Monthly Active Sheets", parsed.monthLabel, result);
    return result;
  });
}
...
function archiveMonthlySheetsBySpecs_(parsedMonth, specs, timing) {
  ...
  markFrameworkStep_(timing, "Archive workbook opened", "Archive Spreadsheet ID: " + archiveId);

  specs.forEach(function(spec) {
    const sheets = findArchiveMonthlyCandidateSheets_(ss, spec.keywords, parsedMonth.monthLabel);
    if (!sheets.length) {
      const message = "No local sheet found for " + spec.label + " " + parsedMonth.monthLabel;
      logBestEffortWarning_(message);
    ...
    sheets.forEach(function(sheet) {
      const sheetName = sheet.getName();
      try {
        copySheetToArchiveAndDeleteLocal_(ss, archiveSs, sheet, sheetName);
        result.archived.push(sheetName);
        markFrameworkStep_(timing, "Archived and removed local sheet: " + sheetName);
      } catch (err) {
...
function copySheetToArchiveAndDeleteLocal_(ss, archiveSs, sourceSheet, archiveName) {
  ...
  deleteSheetSafely_(ss, sourceSheet, "archive monthly local cleanup", [verified && verified.getName()]);
  return verified;
}
...
function formatRawData() {
  ...
  try {
    const outputSheet = formatRawDataInPlaceSheet_(sourceSheet, monthParts, timing, markRuntimeStep_);
    ...
    writeRuntimeTimingReport_(timing);
    notifyErrorWithTiming_("Format Raw Data stopped: " + err.message);
    throw err;
  }
}

function formatRawDataInPlaceSheet_(sourceSheet, monthParts, timing, markStepFn) {
  if (!sourceSheet) {
  ...
  if (sourceSheet.getName() !== outputName) {
    ...
    step(timing, "Rename Raw Data import sheet in place: " + sourceName + " -> " + outputName);
  }
  ...
  hideSheetIfNeeded_(sourceSheet, timing, "Hide Raw Data sheet after formatting: " + sourceSheet.getName());
...
function applyOutputVisibilityPolicy_(sheet, dashboard, sheetType, timing) {
  ...
  const behavior = getBehaviorForSheetType_(dashboard || loadDashboardConfig_(), normalizedSheetType) || {};
...
function createOutputSheetFromDashboardTemplate_(sheetType, outputName, outputRows, firstDay, endDate, timing, timingLabel) {
  ...
  const markCanvasStep = function(label) {
    if (timing) markFrameworkStep_(timing, String(timingLabel || "Dashboard fast canvas") + " - " + label + ": " + outputName);
  };
  ...
  // 1. FAST CANVAS: Insert a perfectly blank sheet (zero overhead)
  const sheet = insertGovernedOutputSheet_(ss, outputName);
  showSheetIfNeeded_(sheet, timing, String(timingLabel || "Dashboard fast canvas") + " - sheet visibility enforced visible: " + outputName);
  markCanvasStep("blank sheet inserted in governed order");
  ...
  // 2. RIGHT-SIZE GRID: Force dimensions before writing to save rendering ticks
  resizeSheetGrid_(sheet, requiredRows, width);
  markCanvasStep("grid resized to " + requiredRows + " rows x " + width + " cols");

  applyGovernedTextAndNumberFormats_(sheet, dashboard, headers, DATA_START_ROW, rows.length);
  markCanvasStep("governed text/date/number formats applied");

  // 3. NAKED WRITE: Set raw values instantly on unformatted cells
  if (rows.length) {
  ...
  markCanvasStep("raw values written | Rows: " + rows.length);

  // 4. BATCH PAINT: Copy top structures, headers, and grid formatting from Template
  try {
    template.getRange(1, 1, DATA_START_ROW - 2, width).copyTo(sheet.getRange(1, 1, DATA_START_ROW - 2, width));
    markCanvasStep("title rows copied from template");
    template.getRange(HEADER_ROW, 1, 1, width).copyTo(sheet.getRange(HEADER_ROW, 1, 1, width));
    markCanvasStep("header row copied from template");

    if (rows.length) {
      template.getRange(DATA_START_ROW, 1, 1, width)
        .copyTo(sheet.getRange(DATA_START_ROW, 1, rows.length, width), SpreadsheetApp.CopyPasteType.PASTE_FORMAT, false);
      markCanvasStep("data-row format copied from template | Rows: " + rows.length);
    } else {
      markCanvasStep("data-row format copy skipped - no rows");
    }

    // Explicitly stamp governed title text variables
    ...
    markCanvasStep("title/date/header values stamped");
  } catch (err) {
    throw new Error("Output style paint failed for " + outputName + ": " + err.message);
  }

  // 5. ENFORCE SYSTEM ENVIRONMENT: Set frozen attributes, filters, and color palettes
  try {
    ...
    markCanvasStep("frozen rows/columns and tab color applied");

    const existingFilter = sheet.getFilter && sheet.getFilter();
    if (existingFilter) existingFilter.remove();
    sheet.getRange(HEADER_ROW, 1, Math.max(sheet.getMaxRows() - HEADER_ROW + 1, 1), width).createFilter();
    markCanvasStep("filter created");
  } catch (err) {
    throw new Error("Environmental configurations failed for " + outputName + ": " + err.message);
  }

  applyUniversalFastCanvasFormatting_(sheet, dashboard, headers);
  markCanvasStep("universal canvas formatting applied");
  clearSheetRuntimeCachesForSheet_(sheet);
  markCanvasStep("runtime caches cleared");
  return sheet;
}
...
function createRawDataOutputSheetFromTemplateFast_(sheetType, outputName, outputRows, firstDay, endDate, sourceName) {
  ...
  if (!template) {
    throw new Error("Template not found: " + sheetDef.templateName);
  }
  ...
  // 3. NAKED WRITE: Set raw values instantly on unformatted cells (~2-3 seconds)
  if (rows.length) {
    sheet.getRange(DATA_START_ROW, 1, rows.length, width).setValues(rows.map(function(row) {
      const out = (row || []).slice(0, width);
      while (out.length < width) out.push("");
  ...
  // 4. BATCH PAINT: Copy title structures, headers, and grid formatting from Template AFTER values exist.
  // STRICT BOUNDARY: template replication failures must stop the workflow.
  template.getRange(1, 1, DATA_START_ROW - 2, width).copyTo(sheet.getRange(1, 1, DATA_START_ROW - 2, width));
  template.getRange(HEADER_ROW, 1, 1, width).copyTo(sheet.getRange(HEADER_ROW, 1, 1, width));

  const rawDataFormatCopyLimit = 2500;
  if (rows.length > 0 && rows.length <= rawDataFormatCopyLimit) {
    template.getRange(DATA_START_ROW, 1, 1, width)
      .copyTo(sheet.getRange(DATA_START_ROW, 1, rows.length, width), SpreadsheetApp.CopyPasteType.PASTE_FORMAT, false);
  } else if (rows.length > rawDataFormatCopyLimit) {
    logBestEffortWarning_("Raw Data template data-row format copy skipped for " + outputName + " because row count " + rows.length + " exceeds " + rawDataFormatCopyLimit + "; governed formats were applied instead.");
  }
  ...
  try {
    ...
    throw new Error("Raw Data environmental configurations failed for " + outputName + ": " + err.message);
  }
...
function ensureOutputSheetHasFormattedRows_(sheet, requiredRows, width) {
  ...
  if (RFF_OUTPUT_EXTEND_TEMPLATE_FORMATTING) {
    try {
      ...
      while (processed < rowsToAdd) {
        const chunk = Math.min(batchChunk, rowsToAdd - processed);
        sheet.getRange(DATA_START_ROW, 1, 1, width)
          .copyTo(sheet.getRange(currentRows + 1 + processed, 1, chunk, width), { formatOnly: true });
        processed += chunk;
      }
    } catch (err) {
...
function formatCarePlanDueOrUnlockedFromDashboard_(sheetType, promptTitle, startDateCellA1, endDateCellA1) {
  return runFrameworkTimed_(promptTitle, function(timing) {
    ...
    const originalStartDate =
      coerceToValidDate_(sourceSheetOriginal.getRange(startDateCellA1).getValue()) ||
      coerceToValidDate_(sourceSheetOriginal.getRange(startDateCellA1).getDisplayValue()) ||
      monthParts.firstDay;
    ...
    const titleInfoText = collectMovedTitleInfoCells_(sourceSheetOriginal, sheetType);
...
function buildDemoPFromScratch() {
  ...
  try {
    ...
    markRuntimeStep_(timing, "Locate validated Raw Data source for Demo P initialization: " + rawSheet.getName());
    ...
    markRuntimeStep_(timing, "Demo P in-memory flat-record contact compression complete | Rows retained: " + flatCount);
    writeRuntimeTimingReport_(timing);
...
function updateDemoPMonthlySync() {
  ...
  try {
    ...
    markRuntimeStep_(timing, "Update Demo P monthly sync workflow complete: " + (demoSheet ? demoSheet.getName() : "No Demo P update"));
    writeRuntimeTimingReport_(timing);
...
function getOrCreateDemoPArchiveSheet_(ss, sourceHeaders, timing, timingPrefix) {
  ...
  if (!sheet) {
    ...
    if (template) {
      ...
      positionSheetInGlobalSortOrder_(sheet);
      markArchiveStep("archive sheet named and positioned");
    } else {
      sheet = insertGovernedOutputSheet_(ss, DEMO_P_ARCHIVE_SHEET);
      markArchiveStep("blank governed archive sheet inserted");
    }
  }
...
function removeActiveDemoPPMRsFromDisenrolledExclusion_(demoSheet) {
  ...
  const rowsToDelete = [];
  exclusionData.values.forEach(function(row, offset) {
    const pmr = normalizePMR_(row[exclusionPmrIdx]);
    if (pmr && activePMRs.has(pmr)) rowsToDelete.push(DATA_START_ROW + offset);
  });

  const deleted = deleteRowNumberBatches_(exclusionSheet, rowsToDelete);
  if (deleted > 0) {
    clearSheetRuntimeCachesForSheet_(exclusionSheet);
    logBestEffortWarning_("Re-Enrollment Engine: Removed " + deleted + " active Demo P PMR rows from Disenrolled Exclusion.");
  }
  return deleted;
}
...
function createActiveDemoPFromRawData_(rawSheet, targetName, monthParts, timing) {
  ...
  const demoSheet = insertGovernedOutputSheet_(ss, targetName);
  demoSheet.showSheet();
  ss.setActiveSheet(demoSheet);
  ...
  applyGovernedTextAndNumberFormats_(demoSheet, dashboard, headers, DATA_START_ROW, workingData.values.length);
  ...
  if (timing) markFrameworkStep_(timing, "Demo P unified values flushed to spreadsheet canvas");
...
function buildMonthlyChangeReportForMonth_(monthParts, timing) {
  ...
  if (reportSheet) {
    deleteSheetSafely_(ss, reportSheet, "Monthly Change report rebuild");
    markRuntimeStep_(timing, "Monthly Change detail - existing report sheet deleted: " + reportName);
  }
  ...
  positionSheetInGlobalSortOrder_(reportSheet);
  showSheetIfNeeded_(reportSheet, timing, "Monthly Change detail - report sheet shown");
  markRuntimeStep_(timing, "Monthly Change report sheet created from template in governed order");
...
function runMonthlyUpdate() {
  ...
  return runFrameworkTimed_("Create Monthly Update " + formatReportDateLabel_(monthParts.firstDay), function(timing) {
    ...
    const masterSheet = createMasterListForMonth_(monthParts, timing);
    ...
    try {
      createIndexSheet();
      const monthlyUpdateSortMoves = enforceOperationalSheetSortOrder_(monthParts);
      markFrameworkStep_(timing, "Create Monthly Update - Index refreshed and active tabs organized without showing hidden system/template sheets | Moves: " + monthlyUpdateSortMoves);
    } catch (err) {
      logBestEffortWarning_("Monthly Update final index/sort skipped: " + err.message);
    }
...
function updateDemoPMonthlySyncForMonth_(monthParts, timing, options) {
  ...
  if (!ss.getSheetByName(mcrName)) {
    const errorMsg = "Update Demo P Aborted: The Monthly Change Report '" + mcrName + "' was not found.\n\nPlease execute 'Build Monthly Change Report' first before syncing changes to Demo P.";
    SpreadsheetApp.getUi().alert("Missing Dependency", errorMsg, SpreadsheetApp.getUi().ButtonSet.OK);
...
function createDisenrolledListForMonth_(monthParts, timing, options) {
  ...
  const exclusionSheet = ss.getSheetByName(DISENROLLED_EXCLUSION_SHEET);
  positionSheetInGlobalSortOrder_(exclusionSheet);
  step("Position Disenrolled Exclusion in governed sort order");
...
function createMasterListForMonth_(monthParts, parentTiming) {
  if (!monthParts) return null;
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  try {
    ...
    if (existingMasterSheet) {
      ...
      markStep("Create naked canvas - Master List output sheet");
    }
    ...
    markStep("Copy Primary PMR rows from processed Demo P to Master List");
    trimOutputSheetToDataSize_(masterSheet, copiedRowCount || 0, SHEET_TYPE.MASTER_LIST);
    markStep("Right-size Master List output rows");
    ...
    markStep("Batch write Care Plan sync data to Master List");
    if (existingMasterSheet) {
      masterSheet = promoteStagedMasterListSheet_(ss, masterSheet, existingMasterSheet, masterName, copiedRowCount || 0, timing, markStep);
    } else {
      setUniqueSheetName_(masterSheet, masterName);
      markStep("Set final Master List sheet name");
    }
...
function createMasterListSheetFromTemplate_(ss, targetName, monthParts, timing, timingLabel) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  const prefix = String(timingLabel || "Master List canvas detail");
  const markCanvasStep = function(label, details) {
    if (timing) markFrameworkStep_(timing, prefix + " - " + label + ": " + targetName, details || "");
  };
  const existing = ss.getSheetByName(targetName);
  markCanvasStep("existing sheet lookup complete", existing ? "Existing sheet found" : "No existing sheet found");
  if (existing) {
    deleteSheetSafely_(ss, existing, "Master List canvas replacement", [RFF_BASE_TEMPLATE_NAME]);
    markCanvasStep("existing target sheet deleted after guard validation");
  }

  const dashboard = loadDashboardConfig_();
  markCanvasStep("dashboard configuration loaded");
  const sheetDef = getSheetDefinitionByType_(dashboard, SHEET_TYPE.MASTER_LIST);
  const headers = getHeadersForSheetType_(dashboard, SHEET_TYPE.MASTER_LIST);
  markCanvasStep("headers resolved | Columns: " + headers.length);
  const template = ss.getSheetByName(sheetDef.templateName);
  markCanvasStep("template lookup complete", sheetDef.templateName);
  if (!template) {
  ...
  const sheet = insertGovernedOutputSheet_(ss, targetName);
  markCanvasStep("blank governed sheet inserted in governed order", "Required rows: " + requiredRows + "; Columns: " + width);
  showSheetIfNeeded_(sheet, timing, prefix + " - sheet visibility enforced visible: " + targetName);
  resizeSheetGrid_(sheet, requiredRows, width);
  markCanvasStep("grid resized", "Rows: " + requiredRows + "; Columns: " + width);
  ...
  try {
    ...
    if (requiredRows >= DATA_START_ROW) {
      ...
      markCanvasStep("data-row format copied from template", "Rows formatted: " + (requiredRows - DATA_START_ROW + 1));
    } else {
      markCanvasStep("data-row format copy skipped - no data rows");
    }
  } catch (err) {
    logBestEffortWarning_("Master List naked canvas style paint skipped: " + err.message);
  }
  applyColumnWidths_(sheet, dashboard, headers);
  markCanvasStep("column widths applied");
  ...
  try {
    ...
    markCanvasStep("title/date values stamped");
    if (headers.length) sheet.getRange(HEADER_ROW, 1, 1, width).setValues([headers]);
    markCanvasStep("header values stamped");
    sheet.setFrozenRows(template.getFrozenRows());
    markCanvasStep("frozen rows applied");
    sheet.setFrozenColumns(Math.min(template.getFrozenColumns(), width));
    markCanvasStep("frozen columns applied");
    sheet.setTabColor(template.getTabColor());
    markCanvasStep("tab color applied");
  } catch (err) {
...
function getIndexSheetNamesInSortOrder_(ss, predicate) {
  if (!ss || typeof predicate !== "function") return [];
  return ss.getSheets()
    .map(function(sheet) { return sheet.getName(); })
    .filter(function(sheetName) { return predicate(sheetName); })
    .sort(compareSheetNamesByGlobalOrder_);
}

function getIndexSheetNamesInMonthSortOrder_(ss, predicate) {
  if (!ss || typeof predicate !== "function") return [];
  return ss.getSheets()
    .map(function(sheet) { return sheet.getName(); })
    .filter(function(sheetName) { return predicate(sheetName); })
    .sort(compareSheetNamesByMonthThenGlobalOrder_);
}

function compareSheetNamesByGlobalOrder_(a, b) {
  const rankDiff = getGlobalSheetSortRankByName_(a) - getGlobalSheetSortRankByName_(b);
  if (rankDiff !== 0) return rankDiff;
  const monthDiff = getSheetMonthSortKey_(b) - getSheetMonthSortKey_(a);
  if (monthDiff !== 0) return monthDiff;
  return String(a || "").localeCompare(String(b || ""));
}

function compareSheetNamesByMonthThenGlobalOrder_(a, b) {
  const monthDiff = getSheetMonthSortKey_(b) - getSheetMonthSortKey_(a);
  if (monthDiff !== 0) return monthDiff;
  const rankDiff = getGlobalSheetSortRankByName_(a) - getGlobalSheetSortRankByName_(b);
  if (rankDiff !== 0) return rankDiff;
...
function isImportOrUnformattedSheetRank_(rank) {
  return rank >= 60 && rank <= 130;
}

function getSheetMonthSortKey_(sheetName) {
  const date = extractFirstDateFromSheetName_(sheetName);
  if (date) return date.getFullYear() * 100 + date.getMonth() + 1;
  const text = String(sheetName || "").toLowerCase();
  const monthNames = ["january", "february", "march", "april", "may", "june", "july", "august", "september", "october", "november", "december"];
  for (let i = 0; i < monthNames.length; i++) {
    const full = monthNames[i];
    const short = full.slice(0, 3);
    if (new RegExp("(^|[^a-z])(" + full + "|" + short + ")([^a-z]|$)").test(text)) return 200000 + i + 1;
  }
  return 0;
}
...
function isUnformattedImportSheetNameForIndex_(sheetName) {
  ...
  return /^(B|CD|UC|RD)\s+/i.test(name);
}
...
function buildIndexRestoreHyperlinkFormula_(targetSheetName) {
...
function createIndexSheet() {
  ...
  addIndexSection_("Current Sheets", getIndexSheetNamesInSortOrder_(ss, function(sheetName) {
    return sheetName === SHEET_TYPE.DEMO_P ||
      sheetName.indexOf(MASTER_LIST_PREFIX) === 0 ||
      sheetName.indexOf(MONTHLY_CHANGE_REPORT_PREFIX) === 0 ||
      sheetName === DISENROLLED_EXCLUSION_SHEET;
  }));

  const rawImportedSheets = getIndexSheetNamesInMonthSortOrder_(ss, function(sheetName) {
    return sheetName.indexOf("Raw Data") === 0 ||
      sheetName.indexOf(BANNER_PREFIX) === 0 ||
      sheetName.indexOf(BANNER_REPORT_ALT_PREFIX) === 0 ||
      sheetName.indexOf(CARE_PLAN_DUE_PREFIX) === 0 ||
      sheetName.indexOf(CARE_PLAN_DUE_DATE_ALT_PREFIX) === 0 ||
      sheetName.indexOf(UNLOCKED_PREFIX) === 0 ||
  ...
  Object.keys(importGroups).sort(function(a, b) { return b.localeCompare(a); }).forEach(function(groupKey) {
    ...
    if (importGroups[groupKey].length > 0) {
      const sampleDate = extractFirstDateFromSheetName_(importGroups[groupKey][0]);
    ...
    importGroups[groupKey].sort(function(a, b) {
      return getGlobalSheetSortRankByName_(a) - getGlobalSheetSortRankByName_(b);
    });
    addIndexSection_(sectionLabel, importGroups[groupKey]);
  });

  addIndexSection_("Unformatted Sheets", getIndexSheetNamesInMonthSortOrder_(ss, function(sheetName) {
    return isUnformattedImportSheetNameForIndex_(sheetName);
  }));

  addIndexSection_("Archive / System Sheets", [
    DEMO_P_ARCHIVE_SHEET,
    "Framework Timing Report",
    "Dashboard Quality Report",
    RFF_DASHBOARD_SHEET
  ...
  const remainingLiveSheets = ss.getSheets()
    ...
    .sort(compareSheetNamesByGlobalOrder_);
  addIndexSection_("Other Live Sheets", remainingLiveSheets);
  ...
  resizeSheetGrid_(sheet, INDEX_FIXED_ROW_COUNT, 10);
  ...
  if (dataRowCount > 0) {
    ...
    for (let r = 5; r <= finalIndexMatrix.length; r++) {
      ...
      if (["Current Sheets", "Unformatted Sheets", "Archive / System Sheets", "System Sheets", "Templates", "Other Live Sheets"].indexOf(label) !== -1 || label.indexOf("Imported Data - ") === 0) {
        sectionRows.push(rowColToA1_(r, 1) + ":" + rowColToA1_(r, 4));
      }
    }
...
function getGlobalSheetSortRankByName_(sheetName) {
  ...
  // 4. Archive slot trails directly behind active Disenrolled Exclusion.
  ...
  const isSystemOrTemplate = (sheetName === "Framework Timing Report" || sheetName === RFF_TIMING_SHEET ||
  ...
  // UNYIELDING SYSTEM / TEMPLATE TAIL-BLOCK HOOKS.
  ...
  if (sheetName === "Template - Banner Report") return 800;
  if (sheetName === "Template - Care Plan Due") return 810;
  if (sheetName === "Template - Unlocked Care Plan") return 820;
  if (sheetName === "Template - Raw Data") return 830;
  if (sheetName === "Template - Demo P") return 840;
  if (sheetName === "Template - Disenrolled Exclusion") return 850;
  if (sheetName === "Template - Master List") return 860;
  if (sheetName === "Template - Monthly Change") return 870;
  if (sheetName === RFF_BASE_TEMPLATE_NAME) return 900;
  if (sheetName.indexOf("Template - ") === 0 || sheetName.indexOf("Template") !== -1) return 890;

  return 190;
}

function isOperationalSheetForSort_(sheetName) {
  const name = String(sheetName || "").trim();
  if (!name) return false;
  if (name === INDEX_SHEET) return true;
  if (name === SHEET_TYPE.DEMO_P || name.indexOf(DEMO_P_PREFIX) === 0) return true;
  if (name.indexOf(MASTER_LIST_PREFIX) === 0) return true;
  if (name.indexOf(MONTHLY_CHANGE_REPORT_PREFIX) === 0) return true;
  if (name === DISENROLLED_EXCLUSION_SHEET || /^Disenrolled\b/i.test(name) && name.indexOf("Template") === -1) return true;
  if (name.indexOf("Raw Data") === 0) return true;
  if (name.indexOf(BANNER_PREFIX) === 0 || name.indexOf(BANNER_REPORT_ALT_PREFIX) === 0) return true;
  if (name.indexOf(CARE_PLAN_DUE_PREFIX) === 0 || name.indexOf(CARE_PLAN_DUE_DATE_ALT_PREFIX) === 0) return true;
  if (name.indexOf(UNLOCKED_PREFIX) === 0 || name.indexOf("Unlocked Care Plan") === 0) return true;
  if (/^B(\b|\s)/i.test(name) || /^CD(\b|\s)/i.test(name) || /^UC(\b|\s)/i.test(name) || /^RD(\b|\s)/i.test(name)) return true;
  return false;
}

function getSystemTemplateTailBlockNames_() {
  return [
    RFF_TIMING_SHEET,
    RFF_TEST_DASHBOARD_SHEET,
    RFF_DASHBOARD_SHEET,
...
function lockSystemTemplateTailBlockOrder_(ss) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  const tailSheets = getSystemTemplateTailBlockNames_()
    .map(function(name) { return ss.getSheetByName(name); })
    .filter(function(sheet) { return !!sheet; });
  if (!tailSheets.length) return 0;

  let moves = 0;
  tailSheets.forEach(function(sheet, index) {
    const targetPosition = ss.getSheets().length - tailSheets.length + index + 1;
    const wasHidden = typeof sheet.isSheetHidden === "function" && sheet.isSheetHidden();
    try {
      if (sheet.getIndex() !== targetPosition) {
        if (wasHidden) sheet.showSheet();
        ss.setActiveSheet(sheet);
        ss.moveActiveSheet(targetPosition);
        moves++;
      }
      if (wasHidden && !sheet.isSheetHidden()) sheet.hideSheet();
    } catch (err) {
      logBestEffortWarning_("System/template tail-block rank lock skipped for " + sheet.getName() + ": " + err.message);
      try {
        if (wasHidden && !sheet.isSheetHidden()) sheet.hideSheet();
      } catch (hideErr) {
        logBestEffortWarning_("System/template tail-block visibility restore skipped for " + sheet.getName() + ": " + hideErr.message);
      }
    }
  });
  return moves;
...
function lockFrameworkTimingReportAsFinalSheet_(ss) {
  return lockSystemTemplateTailBlockOrder_(ss) > 0;
}

function enforceOperationalSheetSortOrder_(monthParts) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  const orderedSheets = ss.getSheets().filter(function(sheet) {
    return isOperationalSheetForSort_(name_(sheet)) && !(typeof sheet.isSheetHidden === "function" && sheet.isSheetHidden());
  }).sort(function(a, b) {
    const rankDiff = getGlobalSheetSortRankByName_(name_(a)) - getGlobalSheetSortRankByName_(name_(b));
    if (rankDiff !== 0) return rankDiff;
  ...
  let moves = 0;
  orderedSheets.forEach(function(sheet, idx) {
    ...
    try {
      ...
      logBestEffortWarning_("Operational sheet sort skipped for " + name_(sheet) + ": " + err.message);
    }
  });
  try {
    const archiveTarget = ss.getSheetByName(DEMO_P_ARCHIVE_SHEET) || ss.getSheetByName("Archive - Demo P");
    if (archiveTarget && typeof archiveTarget.isSheetHidden === "function" && !archiveTarget.isSheetHidden()) {
  ...
  if (lockFrameworkTimingReportAsFinalSheet_(ss)) moves++;
  clearMonthlySheetLookupCache_();
  return moves;
}
...
function enforceGlobalSheetSortOrder_(monthParts) {
  ...
  function rank_(sheet) {
    return getGlobalSheetSortRankByName_(name_(sheet));
  }
  ...
  ss.getSheets().forEach(function(sheet) {
    visibilityByName[name_(sheet)] = typeof sheet.isSheetHidden === "function" && sheet.isSheetHidden();
  });

  const orderedSheets = ss.getSheets().slice().sort(function(a, b) {
    const aRank = rank_(a);
    const bRank = rank_(b);
    const aImportOrUnformatted = isImportOrUnformattedSheetRank_(aRank);
    const bImportOrUnformatted = isImportOrUnformattedSheetRank_(bRank);
    if (aImportOrUnformatted && bImportOrUnformatted) {
      const dateDiffForImportGroup = dateRank_(a) - dateRank_(b);
      if (dateDiffForImportGroup !== 0) return dateDiffForImportGroup;
    }
  ...
  const currentOrderNames = ss.getSheets().map(name_).join("|");
  const desiredOrderNames = orderedSheets.map(name_).join("|");
  if (currentOrderNames === desiredOrderNames) {
    return lockFrameworkTimingReportAsFinalSheet_(ss) ? 1 : 0;
  }

  let position = 1;
  let moves = 0;
  const hiddenSheetsShownForMove = [];
  orderedSheets.forEach(function(sheet) {
    ...
    try {
      if (sheet.getIndex() !== position) {
        ...
        if (wasHidden) {
          sheet.showSheet();
          hiddenSheetsShownForMove.push(sheet);
        }
        ss.setActiveSheet(sheet);
      ...
      logBestEffortWarning_("Global sheet sort skipped for " + sheetName + ": " + err.message);
    }
  });

  hiddenSheetsShownForMove.forEach(function(sheet) {
    const sheetName = name_(sheet);
    try {
      if (visibilityByName[sheetName] === true && !sheet.isSheetHidden()) sheet.hideSheet();
    } catch (err) {
      logBestEffortWarning_("Global sheet sort visibility restore skipped for " + sheetName + ": " + err.message);
    }
  });
...
function positionSheetInGlobalSortOrder_(sheet) {
  if (!sheet) return false;
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const targetName = String(sheet.getName() || "").trim();
  const orderedNames = ss.getSheets()
    .map(function(candidate) { return candidate.getName(); })
    .sort(compareSheetNamesByGlobalOrder_);
  const targetPosition = orderedNames.indexOf(targetName) + 1;
  if (targetPosition <= 0 || sheet.getIndex() === targetPosition) return false;
  if (typeof sheet.isSheetHidden === "function" && sheet.isSheetHidden()) return false;
  try {
    ss.setActiveSheet(sheet);
    ss.moveActiveSheet(targetPosition);
    ...
    logBestEffortWarning_("Sheet positioning skipped for " + targetName + ": " + err.message);
    return false;
  }
}
...
function appendDisenrolledDeltasToExclusionSheet_(exclusionSheet, sourceHeaders, rowsToAppendByPMR, monthParts, timing) {
  ...
  const writeResult = writeDisenrolledAppendPayloadValues_(exclusionSheet, insertStartRow, appendPayload, width);
  step("append payload values written | Rows: " + appendPayload.length + "; Columns written: " + writeResult.columnsWritten + "; Column runs: " + writeResult.runsWritten + "; Blank columns skipped: " + writeResult.blankColumnsSkipped);
...
function runDashboardQualityStartUp() {
  return runFrameworkTimed_("Dashboard Quality Start Up", function(timing) {
    clearDashboardConfigCache_();
    runDashboardQualityTemplateAndFormatSections_(true, false, timing);
    return true;
  });
}
...
function getScriptFunctionForDashboardSection_(sectionTitle) {
  ...
  if (sectionTitle.indexOf("SECTION F") !== -1) return "getDefaultColumnDefinitionRows_()";
  if (sectionTitle.indexOf("SECTION G") !== -1) return "getDefaultSheetHeaderRows_()";
...
function updateFormatDashboardChangelog_(dashboardSheet, timing) {
  ...
  sectionsToTrack.forEach(function(sectionTitle) {
    try {
      const currentRows = readDashboardSectionRows_(dashboardSheet, sectionTitle);
      ...
      const sectionHeader = getFormatDashboardSectionHeaderForChangelog_(dashboardSheet, sectionTitle);
  ...
  if (changed) {
    ...
    if (timing) markFrameworkStep_(timing, "Dashboard Quality Section G cell-differential changelog updated");
  } else {
    saveDashboardQualitySectionRows_(RFF_DASHBOARD_CHANGELOG_KEY, logRows, { deferSheetWrite: true });
    if (timing) markFrameworkStep_(timing, "Dashboard Quality Section G cell-differential changelog unchanged");
  }
  return changed;
}
...
function runDashboardQualityDashboardVerificationSections_(timing, qualitySheet) {
  ...
  const changelogUpdated = updateFormatDashboardChangelog_(dashboard, timing);
  ...
  const sectionPayloads = [
    ...
    { key: RFF_DASHBOARD_VERIFY_BEHAVIORS_KEY, rows: collectFormatDashboardSheetBehaviorVerificationRows_(dashboardIndex), label: "Section D Sheet Behaviors verified" },
    { key: RFF_DASHBOARD_VERIFY_COLUMNS_KEY, rows: collectFormatDashboardColumnDefinitionVerificationRows_(dashboardIndex), label: "Section F Column Definitions verified" },
    { key: RFF_DASHBOARD_VERIFY_HEADERS_KEY, rows: collectFormatDashboardSheetHeaderVerificationRows_(dashboardIndex), label: "Section G Sheet Headers verified" },
    { key: RFF_DASHBOARD_CHANGELOG_KEY, rows: getDashboardQualitySectionRows_(RFF_DASHBOARD_CHANGELOG_KEY), label: "Section G Format Dashboard Changelog verified" }
  ];

  const sectionWrites = [];
  sectionPayloads.forEach(function(item) {
    const previousRows = getDashboardQualitySectionRows_(item.key);
    const changed = !dashboardQualityRowsEqualValues_(previousRows, item.rows) || (item.key === RFF_DASHBOARD_CHANGELOG_KEY && changelogUpdated);
  ...
  if (sectionWrites.length) {
    ...
    markFrameworkStep_(timing, "Dashboard Quality Sections A-G batch written");
  } else {
    markFrameworkStep_(timing, "Dashboard Quality Sections A-G sheet write skipped - unchanged");
  }

  highlightFormatDashboardChangesFromChangelog_(dashboard, timing);
  return true;
}
...
function collectFormatDashboardSheetHeaderVerificationRows_(dashboardSheet) {
  ...
  if (!sectionRows.length) {
    rows.push([RFF_SECTION_HEADERS, "FAIL", "Blank Section", "Format Dashboard Section G has no header definition rows."]);
    return rows;
  }
  ...
  sectionRows.forEach(function(row, index) {
    ...
    if (!sheetType) rows.push([label, "FAIL", "Blank Sheet Type", "Format Dashboard Section G row has no sheet type."]);
    if (!header) rows.push([label, "FAIL", "Blank Header", "Format Dashboard Section G row has no header name."]);
    if (!sheetType || !header) return;
  ...
  return appendDashboardVerificationPassIfNoIssues_(rows, "Sheet Headers", "No missing, blank, duplicate, or invalid Format Dashboard Section G headers found.");
}
...
function collectFormatDashboardColumnDefinitionVerificationRows_(dashboardSheet) {
  ...
  if (!columnRows.length) {
    rows.push([RFF_SECTION_COLUMNS, "FAIL", "Blank Section", "Format Dashboard Section F has no column definition rows."]);
    return rows;
  }
  ...
  columnRows.forEach(function(row, index) {
    ...
    if (!header) rows.push([label, "FAIL", "Blank Column", "Format Dashboard Section F row has no column/header name."]);
    if (!header) return;
  ...
  return appendDashboardVerificationPassIfNoIssues_(rows, "Column Definitions", "No missing, blank, duplicate, or invalid Format Dashboard Section F column definitions found.");
}
...
function collectFormatDashboardSheetBehaviorVerificationRows_(dashboardSheet) {
  ...
  behaviorRows.forEach(function(row, index) {
    ...
    if (!sheetType) rows.push([label, "FAIL", "Blank Sheet Type", "Format Dashboard Section D row has no sheet type."]);
    if (!behavior) rows.push([label, "FAIL", "Blank Behavior", "Format Dashboard Section D row has no behavior setting."]);
    const blanks = collectBlankDashboardCells_(row, width, "Column ");
    if ((sheetType || behavior) && blanks.length) rows.push([label, "WARNING", "Blank Behavior Configuration", blanks.join(", ") + " are blank in Format Dashboard Section D."]);
  });

  return appendDashboardVerificationPassIfNoIssues_(rows, "Sheet Behaviors", "Format Dashboard Section D sheet behavior rows are populated.");
}
...
function runDashboardQualityValidateTemplates() {
  return runFrameworkTimed_("Dashboard Quality Validate Templates", function(timing) {
    runDashboardQualityTemplateAndFormatSections_(false, false, timing);
    markFrameworkStep_(timing, "Dashboard Quality Section F-G template and changelog sections updated");
    return true;
  });
}
...
function runDashboardQualityTemplateAndFormatSections_(includeDashboardAudit, includeHealthAndSignoff, timing) {
  const dashboard = loadDashboardConfig_(true);
  const qualitySheet = ensureDashboardQualityReportSheet_();
  showSheetIfNeeded_(qualitySheet, timing, "Dashboard Quality surface verified");

  // OPTIMIZATION LATCH: Skip shell signature generation and costly structural shell resets on steady-state workflow loops.
  const shellMatches = true;
  if (!shellMatches) {
    rebuildDashboardQualitySheetShellStructure_(qualitySheet, dashboard, timing);
  } else {
    markFrameworkStep_(timing, "Dashboard Quality matrix shell structure matches validation signatures; layout clear bypassed.");
  ...
  const changelogUpdated = updateFormatDashboardChangelog_(dashboard.sectionIndex.sheet, timing);
  ...
  if (stagedSectionWrites.length) {
    replaceDashboardQualitySectionsRows_(qualitySheet, stagedSectionWrites);
  ...
  if (includeDashboardAudit) {
    runDashboardQualityDashboardVerificationSections_(timing, qualitySheet);
  }
  ...
  if (includeHealthAndSignoff) {
    ...
    runDashboardQualitySectionIfDue_("Signoff", "Section Q Signoff", updateDashboardQualitySignoffSection_, timing);
  }
...
function runOperationalDataPipelineValidations_(timing, qualitySheet) {
  ...
  runDashboardQualitySectionIfDue_(RFF_MASTER_LIST_HEALTH_KEY, "Section J Raw Data Validation", runDashboardQualityRawDataValidation_, timing);
  ...
  runDashboardQualitySectionIfDue_(RFF_DEMO_P_PROCESSING_VALIDATION_KEY, "Section M Demo P Quality Validation", runDashboardQualityDemoPValidation_, timing);
  runDashboardQualitySectionIfDue_(RFF_DISENROLLED_EXCLUSION_VALIDATION_KEY, "Section N Disenrolled Exclusion Validation", runDashboardQualityDisenrolledExclusionValidation_, timing);
  runDashboardQualitySectionIfDue_(RFF_MONTHLY_CHANGE_VALIDATION_KEY, "Section O Monthly Change Validation", runDashboardQualityMonthlyChangeValidation_, timing);
...
function runDashboardQualityFull() {
  return runFrameworkTimed_("Dashboard Quality Workflow", function(timing) {
    ...
    // FAST PATH: Disable heavy health/signoff structural audits on workflow loops.
    const qualitySheet = runDashboardQualityTemplateAndFormatSections_(true, false, timing) || ensureDashboardQualityReportSheet_();
...
const RFF_DASHBOARD_QUALITY_SECTIONS = [
  ...
  { key: RFF_DASHBOARD_CHANGELOG_KEY, title: "SECTION G - FORMAT DASHBOARD CHANGELOG" },
  ...
  { key: RFF_MASTER_LIST_HEALTH_KEY, title: "SECTION J - RAW DATA VALIDATION" },
  ...
  { key: RFF_DEMO_P_PROCESSING_VALIDATION_KEY, title: "SECTION M - DEMO P QUALITY VALIDATION" },
  { key: RFF_DISENROLLED_EXCLUSION_VALIDATION_KEY, title: "SECTION N - DISENROLLED EXCLUSION VALIDATION" },
  { key: RFF_MONTHLY_CHANGE_VALIDATION_KEY, title: "SECTION O - MONTHLY CHANGE VALIDATION" },
...
function getDefaultDashboardQualityDetailHeader_(section) {
  ...
  if (title.indexOf("SECTION G - ") === 0) return ["Timestamp", "Section Changed", "Column Changed", "Previous Value", "New Value", "", ""];
...
function initializeSystemSheets_() {
  ...
  // 1. Build Format Dashboard first.
  let dashboardSheet = ss.getSheetByName(RFF_DASHBOARD_SHEET);
  if (!dashboardSheet) {
    dashboardSheet = ss.insertSheet(RFF_DASHBOARD_SHEET, 1);
  }

  // 2. Build Framework Timing Report second.
  let timingSheet = ss.getSheetByName(RFF_TIMING_SHEET);
  if (!timingSheet) {
    timingSheet = ss.insertSheet(RFF_TIMING_SHEET, 2);
  }

  // 3. Build Dashboard Quality Report third.
  let qualitySheet = ss.getSheetByName(RFF_TEST_DASHBOARD_SHEET);
  if (!qualitySheet) {
    qualitySheet = ss.insertSheet(RFF_TEST_DASHBOARD_SHEET, 3);
  }
...
function setupSystemSheets() {
  ...
  forceBaseTemplateHidden_();
  notify_("System sheets setup complete. System/template tabs are organized.");
  return result;
}
