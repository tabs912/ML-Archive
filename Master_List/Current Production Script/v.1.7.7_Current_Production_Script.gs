...
const MASTER_LIST_MERGE_ML_VERSION = "1.7.7";
...
function getDefaultSheetDefinitionByType_(sheetType) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    return {
      ...
      testRows: numberOrDefault_(row[12], 100),
      useTestRows: parseBoolean_(row[13])
    };
  }
...
function getTemplateDrivenActiveSheetContext_(ss, sheetType) {
  ...
  return { sheetDef: sheetDef, template: template, headers: headers };
}
...
function preflightMonthlyUpdateForMonth_(monthParts, timing) {
  ...
  if (timing) {
    markFrameworkStep_(timing, "Create Monthly Update preflight passed", "Current Raw Data: " + currentRaw.getName() + "; Previous Raw Data: " + previousRaw.getName() + "; Demo P: " + demoSheet.getName());
  }
  return { currentRaw: currentRaw, previousRaw: previousRaw, demoSheet: demoSheet };
}
...
function getConfiguredSheetCreationIndex_(ss, sheetName, ignoreSheetId) {
  ...
  if (!targetRecord.tail) {
    let targetIndex = Math.min(firstTailIndex, sheets.length + 1);
...
function formatBannerReport() {
  return runFrameworkTimed_("Format Banner Report", function(timing) {
    ...
    const outputName = buildBannerReportOutputName_(reportDate);
    assertActiveBannerSourceSheet_(rawSheetOriginal, "Format Banner Report");
    const context = getTemplateDrivenActiveSheetContext_(ss, SHEET_TYPE.BANNER);
    const templateName = context.sheetDef.templateName;
...
function createOutputSheetFromDashboardTemplate_(sheetType, outputName, outputRows, firstDay, endDate, timing, timingLabel) {
  ...
  markCanvasStep("blank sheet inserted in configured creation order");
  ...
  applyTemplateColumnWidths_(sheet, template, width);
  markCanvasStep("template column widths applied");
  ...
  sheet.getRange(DATA_START_ROW, 1, Math.max(sheet.getLastRow() - DATA_START_ROW + 1, 1), Math.max(sheet.getLastColumn(), 1)).setWrapStrategy(SpreadsheetApp.WrapStrategy.CLIP);
  markCanvasStep("template-driven canvas wrap applied");
  clearSheetRuntimeCachesForSheet_(sheet);
  markCanvasStep("runtime caches cleared");
  return sheet;
}

function createRawDataOutputSheetFromTemplateFast_(sheetType, outputName, outputRows, firstDay, endDate, sourceName, timing) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const normalizedSheetType = normalizeDashboardSheetTypeKey_(sheetType);
  const context = getTemplateDrivenActiveSheetContext_(ss, normalizedSheetType);
  const sheetDef = context.sheetDef;
  ...
  const markCanvasStep = function(label, details) {
    if (timing) markFrameworkStep_(timing, "Raw Data fast canvas - " + label + ": " + outputName, details || "");
  };
  ...
  if (rows.length > 0 && rows.length <= rawDataFormatCopyLimit) {
    ...
    logBestEffortWarning_("Raw Data template data-row format copy skipped for " + outputName + " because row count " + rows.length + " exceeds " + rawDataFormatCopyLimit + "; template title/header formatting and column widths were still applied.");
  }
...
function createActiveDemoPFromRawData_(rawSheet, targetName, monthParts, timing) {
  ...
  applyTemplateColumnWidths_(demoSheet, template, width);
...
function runMonthlyUpdate() {
  ...
  return runFrameworkTimed_("Create Monthly Update " + formatReportDateLabel_(monthParts.firstDay), function(timing) {
    preflightMonthlyUpdateForMonth_(monthParts, timing);
    const messages = [];
...
function createMasterListSheetFromTemplate_(ss, targetName, monthParts, timing, timingLabel) {
  ...
  const templateName = MASTER_LIST_TEMPLATE_SHEET;
  const template = ss.getSheetByName(templateName);
  markCanvasStep("template lookup complete", templateName);
  if (!template) {
    throw new Error("Template not found: " + templateName + ". Run Create / Refresh Master List Template first.");
  }

  const headers = getHeaders_(template, HEADER_ROW);
  markCanvasStep("headers resolved from template | Columns: " + headers.length);
  if (!headers.some(function(header) { return String(header || "").trim(); })) {
    throw new Error("Template not usable: " + templateName + " is missing header values on row " + HEADER_ROW + ".");
  }
  ...
  markCanvasStep("blank sheet inserted in configured creation order", "Required rows: " + requiredRows + "; Columns: " + width);
  ...
  for (let col = 1; col <= width; col++) {
    templateColumnWidths.push(template.getColumnWidth(col));
  }
  applyColumnWidthsInRuns_(sheet, templateColumnWidths);
  markCanvasStep("template column widths applied");
...
function isUnformattedImportSheetNameForIndex_(sheetName) {
  ...
  return name.indexOf("Raw Data - Banners") === 0 ||
    name.indexOf("Raw Data - Banner Report") === 0 ||
    name.indexOf("Raw Data - CP Due") === 0 ||
    name.indexOf("Raw Data - Care Plan Due") === 0 ||
    name.indexOf("Raw Data - Unlock CP") === 0 ||
    name.indexOf("Raw Data - Unlocked") === 0;
}
...
function getSheetSortProfileByName_(sheetName) {
  const name = String(sheetName || "").trim();
  const isTemplate = name.indexOf("Template - ") === 0 || name.indexOf("Template") !== -1;
  if (name === INDEX_SHEET) return { rank: 5, group: "index", operational: true, tail: false };
  if (name === SHEET_TYPE.DEMO_P || name === DEMO_P_PREFIX) return { rank: 10, group: "demo_p", operational: true, tail: false };
  if (name.indexOf(MASTER_LIST_PREFIX) === 0) return { rank: 20, group: "master_list", operational: true, tail: false };
  if (name.indexOf(MONTHLY_CHANGE_REPORT_PREFIX) === 0) return { rank: 30, group: "monthly_change", operational: true, tail: false };
  if (name === DISENROLLED_EXCLUSION_SHEET || (/^Disenrolled\b/i.test(name) && !isTemplate)) return { rank: 40, group: "disenrolled", operational: true, tail: false };
  if (name.indexOf("Raw Data ") === 0 && name.indexOf("Raw Data - ") === -1) return { rank: 50, group: "raw_data", operational: true, tail: false };
  if (name.indexOf(BANNER_PREFIX) === 0 || name.indexOf(BANNER_REPORT_ALT_PREFIX) === 0) return { rank: 60, group: "banner", operational: true, tail: false };
  if (name.indexOf(CARE_PLAN_DUE_PREFIX) === 0 || name.indexOf(CARE_PLAN_DUE_DATE_ALT_PREFIX) === 0) return { rank: 70, group: "care_plan_due", operational: true, tail: false };
  if (name.indexOf(UNLOCKED_PREFIX) === 0 || name.indexOf("Unlocked Care Plan") === 0) return { rank: 80, group: "unlocked", operational: true, tail: false };
  if (name.indexOf("Raw Data - Banners") === 0 || name.indexOf("Raw Data - Banner Report") === 0) return { rank: 90, group: "raw_banner_import", operational: true, tail: false };
  if (name.indexOf("Raw Data - CP Due") === 0 || name.indexOf("Raw Data - Care Plan Due") === 0) return { rank: 100, group: "raw_care_plan_import", operational: true, tail: false };
  if (name.indexOf("Raw Data - Unlock CP") === 0 || name.indexOf("Raw Data - Unlocked") === 0) return { rank: 110, group: "raw_unlocked_import", operational: true, tail: false };
  if (/^B(\b|\s)/i.test(name) || /^CD(\b|\s)/i.test(name) || /^UC(\b|\s)/i.test(name) || /^RD(\b|\s)/i.test(name)) return { rank: 120, group: "short_import", operational: true, tail: false };
  if (name === DEMO_P_ARCHIVE_SHEET || name.indexOf("Archive - Demo P") === 0) return { rank: 130, group: "demo_p_archive", operational: true, tail: false, forceHidden: true };

  if (name === RFF_TIMING_SHEET || name === "Framework Timing Report") return { rank: 500, group: "framework_timing", operational: false, tail: true };
  if (name === RFF_TEST_DASHBOARD_SHEET || name === "Dashboard Quality Report") return { rank: 510, group: "dashboard_quality", operational: false, tail: true };
  if (name === RFF_DASHBOARD_SHEET) return { rank: 520, group: "format_dashboard", operational: false, tail: true };
  if (name === "Template - Banner Report") return { rank: 800, group: "template", operational: false, tail: true };
  if (name === "Template - Care Plan Due") return { rank: 810, group: "template", operational: false, tail: true };
  if (name === "Template - Unlocked Care Plan") return { rank: 820, group: "template", operational: false, tail: true };
  if (name === "Template - Raw Data") return { rank: 830, group: "template", operational: false, tail: true };
  if (name === "Template - Demo P") return { rank: 840, group: "template", operational: false, tail: true };
  if (name === "Template - Disenrolled Exclusion") return { rank: 850, group: "template", operational: false, tail: true };
  if (name === "Template - Master List") return { rank: 860, group: "template", operational: false, tail: true };
  if (name === "Template - Monthly Change") return { rank: 870, group: "template", operational: false, tail: true };
  if (isTemplate) return { rank: 890, group: "template_other", operational: false, tail: true };
  if (name === RFF_BASE_TEMPLATE_NAME) return { rank: 900, group: "base_template", operational: false, tail: true, forceHidden: true };

  return { rank: 190, group: "other", operational: false, tail: false };
}
...
function buildSheetSortRecord_(sheet, currentFirst) {
  const name = String(sheet && sheet.getName ? sheet.getName() : "").trim();
  const profile = getSheetSortProfileByName_(name);
  const date = extractFirstDateFromSheetName_(name);
  const dateMs = date ? date.getTime() : 0;
  const currentMonthBoost = currentFirst && dateMs === currentFirst.getTime() ? -9999999999999 : 0;
  return {
    sheet: sheet,
    name: name,
    rank: profile.rank,
    operational: profile.operational === true,
    tail: profile.tail === true,
    forceHidden: profile.forceHidden === true,
    monthSort: currentMonthBoost || (dateMs ? -dateMs : 0)
  };
}
...
function compareSheetSortRecords_(a, b) {
  ...
  if (rankDiff !== 0) return rankDiff;
  const monthDiff = a.monthSort - b.monthSort;
  if (monthDiff !== 0) return monthDiff;
...
function getOrderedSheetSortRecords_(ss, monthParts, options) {
  ...
  return ss.getSheets()
    .map(function(sheet) { return buildSheetSortRecord_(sheet, currentFirst); })
...
function enforceSystemTemplateTailBlockOrder_(ss) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  const recordsByName = {};
  getOrderedSheetSortRecords_(ss).forEach(function(record) {
    if (record.tail) recordsByName[record.name] = record;
  });
  const tailRecords = getSystemTemplateTailBlockNames_()
    .map(function(name) { return recordsByName[String(name || "").trim()]; })
    .filter(function(record) { return !!record; });
  if (!tailRecords.length) return 0;
  const startPosition = Math.max(1, ss.getNumSheets() - tailRecords.length + 1);
  const moves = applySheetSortRecords_(ss, tailRecords, startPosition, "System/template tail-block sort");
  tailRecords.forEach(function(record) {
    if (!record.forceHidden) return;
    try {
      if (!record.sheet.isSheetHidden()) {
        activateVisibleSheetBeforeHiding_(record.sheet);
        record.sheet.hideSheet();
      }
    } catch (err) {
      logBestEffortWarning_("System/template tail-block hide enforcement skipped for " + record.name + ": " + err.message);
    }
  });
  return moves;
}
...
function enforceGlobalSheetSortOrder_(monthParts) {
  ...
  const moves = applySheetSortRecords_(ss, records, 1, "Global sheet sort") + enforceSystemTemplateTailBlockOrder_(ss);
